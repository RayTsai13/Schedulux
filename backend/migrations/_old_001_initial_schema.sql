...
CREATE TABLE services (
    ...
    buffer_time_minutes INTEGER DEFAULT 0 CHECK (buffer_time_minutes >= 0), -- extra time between apts
    price DECIMAL(10,2),
...
CREATE TABLE schedule_rules (
    ...
    -- Time specifications (only fill relevant fields based on rule_type and leave others as NULL)
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        INSERT INTO schedule_rules_history (schedule_rule_id, action, rule_data, changed_at)
    ...
    ELSIF TG_OP = 'DELETE' THEN
        INSERT INTO schedule_rules_history (schedule_rule_id, action, rule_data, changed_at)
        VALUES (OLD.id, 'deleted', row_to_json(OLD), CURRENT_TIMESTAMP);
        RETURN OLD;
    END IF;
    RETURN NULL;
END;
...
-- Appointment slot booking count triggers
