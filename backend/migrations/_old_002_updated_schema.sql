...
CREATE FUNCTION public.track_schedule_rule_changes() RETURNS trigger
    ...
    AS $$
DECLARE
    rule_id_to_use INTEGER;
BEGIN
    IF TG_OP = 'INSERT' THEN
        rule_id_to_use := NEW.id;
        INSERT INTO schedule_rules_history (schedule_rule_id, action, rule_data)
        SELECT rule_id_to_use, 'created', row_to_json(NEW.*);
        RETURN NEW;
    ELSIF TG_OP = 'UPDATE' THEN
        rule_id_to_use := NEW.id;
        INSERT INTO schedule_rules_history (schedule_rule_id, action, rule_data)
        SELECT rule_id_to_use, 'updated', row_to_json(NEW.*);
        RETURN NEW;
    ELSIF TG_OP = 'DELETE' THEN
        rule_id_to_use := OLD.id;
        INSERT INTO schedule_rules_history (schedule_rule_id, action, rule_data)
        SELECT rule_id_to_use, 'deleted', row_to_json(OLD.*);
        RETURN OLD;
    END IF;
    RETURN NULL;
END;
...
--
-- Name: schedule_rules_history schedule_rules_history_schedule_rule_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.schedule_rules_history
    ADD CONSTRAINT schedule_rules_history_schedule_rule_id_fkey FOREIGN KEY (schedule_rule_id) REFERENCES public.schedule_rules(id) ON DELETE CASCADE;
